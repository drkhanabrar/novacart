...
import { trackEvent } from "@/components/NovaTracker";

/*
 * Plain numbers only.
 *
 * These fields were typed `any`, which let a raw Prisma record be passed
 * straight in from a server component. Prisma returns Decimal objects for money
 * columns, and Decimal cannot cross the server/client boundary - React can only
 * serialise plain values.
 *
 * Typing them as number means the conversion has to happen on the server, where
 * it belongs, and TypeScript now catches the mistake instead of the browser.
 */
interface AddToCartProps {
  product: {
    ...
    title: string;
    basePrice: number;
    variants?: { id: string; price: number; imageUrl?: string | null }[];
  };
}

...
export default function CheckoutClient({
  ...
  async function placeOrder() {
    ...
    try {
      ...
      /*
       * createRazorpayOrder() returns a discriminated result.
       * When success is false, stop here.
       * After this check TypeScript knows that the remaining
       * Razorpay fields are defined.
       */
      if (!result.success) {
        setError(result.error);
        return;
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response) => {
          ...
          /*
           * result.orderId is guaranteed by the success branch above,
           * but keeping this guard makes the payment completion path
           * defensive.
           */
          if (!result.orderId) {
            setError(
  ...
  if (orderId) {
    return (
      <div className="mx-auto max-w-2xl px-5 py-20 text-center">
        ...
        <span className="section-kicker mt-6 inline-block">
          Order confirmed
        </span>
        ...
        <p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-ink-soft">
          ...
          has been confirmed. We&apos;ll use your saved contact details for
          order updates.
        </p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Link
            href="/products"
            className="rounded-full border border-ink/10 px-5 py-3 text-sm font-bold text-slate-700"
          >
            Continue shopping
          </Link>
        </div>
    ...
    <div className="mx-auto max-w-[1180px] px-5 py-10 sm:px-6 lg:py-16">
      <div className="mb-10">
        <div className="mb-7 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em]">
          <span className="rounded-full bg-poppy px-3 py-1.5 text-white">
            1 Delivery
          </span>

          <span className="h-px w-6 bg-ink/10" />

          <span className="rounded-full border border-ink/10 bg-card px-3 py-1.5 text-ink-soft">
            2 Payment
          </span>

          <span className="h-px w-6 bg-ink/10" />

          <span className="hidden rounded-full border border-ink/10 bg-card px-3 py-1.5 text-ink-soft sm:inline-flex">
            3 Confirmation
          </span>
        </div>
        ...
        <p className="mt-2 text-sm text-ink-soft">
          Review delivery and payment details before placing your order.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        <section className="space-y-6">
          <div className="premium-surface rounded-3xl p-6 sm:p-8">
            <div className="flex items-start justify-between gap-4">
              ...
              <Link
                href="/account"
                className="text-xs font-bold text-poppy"
              >
                Manage addresses
              </Link>
            </div>
            ...
            {user.addresses?.length ? (
                  ...
                  <label
                    ...
                    className={`flex cursor-pointer gap-4 rounded-2xl border p-4 transition ${
                      selectedAddress === address.id
                        ...
                        : "border-ink/10 bg-card hover:border-slate-300"
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    <span className="min-w-0">
                      <span className="flex items-center gap-2 text-sm font-bold text-ink">
                        ...
                        {address.isDefault && (
                          <span className="rounded-full bg-emerald-50 px-2 py-1 text-[9px] uppercase tracking-wider text-emerald-700">
                            Default
                          </span>
                        )}
                      </span>
              ...
              <div className="mt-6 rounded-2xl border border-dashed border-slate-300 p-7 text-center">
                ...
                <Link
                  href="/account"
                  className="mt-4 inline-flex text-xs font-bold text-poppy"
                >
                  Open account →
                </Link>
              </div>
        ...
        <aside className="premium-surface h-max rounded-3xl p-6 sm:p-7 lg:sticky lg:top-28">
          ...
          <button
            ...
            className="premium-button mt-6 w-full disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loading
              ? "Preparing payment…"
              : paymentMethod === "RAZORPAY"
          ...
          <div className="mt-5 flex gap-2 text-[11px] leading-5 text-ink-soft/70">
            ...
            <span>
              Payments are verified server-side before an

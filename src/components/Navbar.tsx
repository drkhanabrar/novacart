    ...
    <>
      <header className="sticky top-0 z-50 w-full border-b border-ink/10 bg-cream/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center gap-4 px-5 sm:px-6">
          ...
          <nav
            className="ml-2 hidden items-center gap-6 xl:flex"
          ...
          <div className="ml-auto flex items-center gap-2.5">
            ...
            {/* CONTACT BUTTON — beside profile */}
            <Link
              href="/contact"
              aria-label="Contact NovaCart"
              className="hidden sm:inline-flex h-10 items-center gap-2 rounded-full border border-ink/10 bg-card px-3.5 text-ink-soft shadow-sm transition hover:border-poppy/30 hover:text-poppy"
            >
              <MessageCircle
                className="h-4 w-4"
                strokeWidth={1.8}
            ...
            {user ? (
              <div className="relative hidden sm:block">
                ...
                >
                  ...
                  <span className="hidden max-w-[140px] truncate text-sm font-semibold md:block">
                    {shortName}
                  </span>
                  ...
                  <>
                    ...
                    <div className="absolute right-0 top-[calc(100%+10px)] z-50 w-72 overflow-hidden rounded-3xl border border-ink/10 bg-card shadow-[0_24px_80px_rgba(35,25,18,0.16)]">
                      ...
                      <div className="p-2">
                        ...
                        >
                          <UserRound className="h-4 w-4 text-poppy" />
                          My account
                        </Link>
                        ...
                        <Link
                          ...
                          className="account-menu-link"
                        >
                          <Package className="h-4 w-4 text-poppy" />
                          My orders
                        </Link>
                        ...
                        <Link
                          ...
                          className="account-menu-link"
                        >
                          <MessageCircle className="h-4 w-4 text-poppy" />
                          Contact us
                        </Link>
                      </div>
              ...
              <div className="hidden items-center gap-1.5 sm:flex">
                <Link
                  href="/login"
                  className="px-3 py-2 text-sm font-semibold text-ink-soft transition-colors hover:text-ink"
                >
                  Sign in
                </Link>
                ...
                <Link
                  href="/register"
                  className="inline-flex rounded-xl bg-poppy px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-poppy-dark"
                >
                  Register
                </Link>
              </div>
            ...
            <button
              type="button"
              className="grid h-10 w-10 place-items-center rounded-full border border-ink/10 bg-card text-ink-soft sm:hidden"
              onClick={() =>
                setMobileOpen((v) => !v)
              }
              aria-label={
              ...
              >
                ...
                {user ? (
                  <>
                    ...
                    <button
                      onClick={handleLogout}
                      className="mobile-nav-link text-left text-poppy"
                    >
                      {loggingOut
                        ? "Signing out…"
                        : "Sign out"}

...
export default async function LandingPage() {
   ...
   *
   * The lifecycle exposure value is the lever NOVA uses to act on what it has
   * learned. A SCALING product earns 95, a DECLINING one drops to 25, and the
   * homepage reflects that instead of showing whatever was published most
   * recently. Products with no lifecycle row yet fall back to a neutral 60, so
   * anything published before this existed still appears normally.
   */
  const merchandised = recentProducts
    .slice()
    .sort((a, b) => {
      const exposureA = a.lifecycle?.exposure ?? 60;
      const exposureB = b.lifecycle?.exposure ?? 60;
  ...
  const bestSellers = merchandised.slice(0, 6);

  const heroProduct = merchandised[0] ?? null;

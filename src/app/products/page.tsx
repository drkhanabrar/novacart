...
export default async function ProductsCatalogPage({
  ...
  /*
   * "Best" now means what NOVA currently believes, not only what it believed at
   * publication. Lifecycle exposure leads, because that value already folds in
   * observed performance, and the research score breaks ties within a band.
   *
   * Explicit price sorts are left untouched — a customer who asked for cheapest
   * first should get exactly that.
   */
  const sortedProducts =
    bestFirst
      ? products
          ...
          .sort((a, b) => {
            const exposureA =
              a.lifecycle?.exposure ??
              60;

            const exposureB =
              b.lifecycle?.exposure ??
              60;

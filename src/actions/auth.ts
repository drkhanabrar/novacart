...
// Preserves the existing registration/login/reset/address actions. :contentReference[oaicite:1]{index=1}
...
function registrationAddress(formData: FormData) {
  return {
    label:
      String(
        formData.get("addressLabel") ||
          "Home",
      ).trim() || "Home",

    fullName: String(
      formData.get("addressFullName") || "",
    ).trim(),

    phone: String(
      formData.get("addressPhone") || "",
    ).trim(),
...
function validateAddress(
  ...
  if (
    ...
    return "Please enter a valid delivery phone number.";
  }
...
export async function createAddress(
  ...
  const data =
    savedAddressData(formData);

  const error =
    validateSavedAddress(data);
...
export async function requestPasswordReset(
  ...
  const resetUrl =
    `${baseUrl}/reset-password?token=${rawToken}&email=${encodeURIComponent(email)}`;
